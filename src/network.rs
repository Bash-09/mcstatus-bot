...
pub enum ServerState {
    ...
    pub async fn status(stream: &mut TcpStream) -> Result<StatusResponse, Box<dyn Error + Send + Sync>> {
    ...
    /// * `None` if it is not
    async fn send_packet(stream: &mut TcpStream, packet: DecodedPacket) -> Result<bool, Box<dyn Error + Send + Sync>> {
